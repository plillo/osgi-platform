...
public class TokenAuthenticationBroker implements Broker {
	...
	public TokenAuthenticationBroker(Broker next, Map<String, String> authConfig) {
		this.next = next;
    ...
    public void addConnection(ConnectionContext context, ConnectionInfo info) throws Exception {
		...
		if (response == null) {
			...
			logger.info(">>Connection with token: [{}]. Allowing connection", token);
		}
	...
	public Subscription addConsumer(ConnectionContext context, ConsumerInfo info) throws Exception {
		String host = authConfig.get("host");
		int port = Integer.parseInt(authConfig.get("port"));
		String api_consumer = authConfig.get("api-consumer");
		
		// GET consumer authorization from authenticator
		// TODO
		
		String token = context.getUserName();
		String pname = info.getDestination().getPhysicalName();
		String qname = info.getDestination().getQualifiedName();
		logger.info(">>Consumer: dest(phi,qua): [{},{}] token: [{}]", pname, qname, token);
	...
	public void addProducer(ConnectionContext context, ProducerInfo info) throws Exception {
		String host = authConfig.get("host");
		int port = Integer.parseInt(authConfig.get("port"));
		String api_producer = authConfig.get("api-producer");
		
		// GET producer authorization from authenticator
		// TODO
		
		String token = context.getUserName();
		String pname = info.getDestination().getPhysicalName();
		String qname = info.getDestination().getQualifiedName();
		logger.info(">>Producer: dest(phi,qua): [{},{}] token: [{}]", pname, qname, token);
